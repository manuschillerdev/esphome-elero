...
_MONGOOSE_BASE_URL = f"https://raw.githubusercontent.com/cesanta/mongoose/{_MONGOOSE_VERSION}"
...
# Committed with its source so Git installs do not depend on release assets.
# Rebuild and commit it whenever the frontend changes.
...
def _ensure_ui_header():
    """Require the frontend bundled with this component revision."""
    if _UI_HEADER.is_file() and _UI_HEADER.stat().st_size > 0:
        return
    raise EsphomeError(
        f"Missing or empty bundled frontend: {_UI_HEADER}. "
        "Refresh the external component checkout. For local development, run "
        "mise exec -- pnpm --dir components/elero_web/frontend/app install --frozen-lockfile "
        "and mise exec -- pnpm --dir components/elero_web/frontend/app build."
    )
