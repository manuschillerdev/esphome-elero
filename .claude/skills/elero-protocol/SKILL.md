...
| Offset | Length | Field | Config/C++ | Description |
|--------|--------|-------|------------|-------------|
| 0 | 1 | `pck_len` | - | Packet length (0x1B=27, 0x1C=28, 0x1D=29, 0x1E=30) |
| 1 | 1 | `counter` | `counter` | Rolling message counter (increments each TX) |
| 2 | 1 | `msg_type` | `type` | Message type: 0x44=button, 0x6A=command, 0xCA/0xC9=status |
| 3 | 1 | `type2` | `type2` | Secondary type byte (usually 0x00) |
| 4 | 1 | `hop` | `hop` | Hop count/routing info (usually 0x0a) |
| 5 | 1 | `syst` | - | System address (fixed: 0x01) |
| 6 | 1 | `channel` | `channel` | RF channel (0x11 for channel 1, else channel number) |
| 7-9 | 3 | `src_addr` | `src_address` | Source address (remote address, big-endian) |
...
| 17-19 | 3 | `dst_addr` | `dst_address` | Destination blind address (big-endian) |
| 20+ | var | `payload` | `payload[]` | Encrypted payload (format depends on msg_type) |
